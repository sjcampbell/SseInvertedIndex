...
object BuildInvertedIndex extends Tokenizer{
    ...
    def main(argv: Array[String]) {
        ...
        val termDocIds = data.flatMap(pair => {
            // document ID:  pair._1
            // line:         pair._2
        ...
        val groupedDocIds = termDocIds.reduceByKey(_ ++ _)
        
        // Encrypt key words for the postings list. Each encrypted label will have a group of document IDs 
        // assigned to it. Ths group size is determined by an input parameter.
        val documentGroupSize = args.docGroupSize()
        val encryptedWords = groupedDocIds.flatMap(wordDocPostings => {
            ...
            while(i < wordDocPostings._2.length - 1) {
                ...
                val postingsGroup = new Array[Long](documentGroupSize)
                for (j <- 0 to (documentGroupSize - 1)) {
                    if (j > groupSize - 1) postingsGroup(j) = -1
                    else postingsGroup(j) = wordDocPostings._2(i + j).get()
                }
                
                val docArray = new LongArrayWritable(postingsGroup)
